...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    True
    """
    return accumulate(combiner, base, n, lambda x: term(x) if pred(x) else 1 if combiner == mul else 0)
...
def pingpong(n):
    ...
    def count_up(value, k):
    	if k == n:
    		return value
    	elif k % 7 == 0 or has_seven(k):
    		return count_down(value - 1, k + 1)
    	else:
    		return count_up(value + 1, k + 1)

    def count_down(value, k):	
    	if k == n:
    		return value 
    	elif k % 7 == 0 or has_seven(k):
    		return count_up(value + 1, k + 1)
...
def count_change(amount):
    ...
    greatest_power = 0
    while(amount > 2**(greatest_power + 1)):
    	greatest_power += 1
    greatest_coin = 2**greatest_power
    def split(amount, greatest_coin):
    	if amount == 0:
    		return 1
    	elif amount < 0:
    		return 0
    		...
    		return split(amount - greatest_coin, greatest_coin) + split(amount, greatest_coin // 2)
    	return split(amount, greatest_coin)
